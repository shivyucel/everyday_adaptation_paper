...
def detect(t, temp, pctile=90, minDuration=2):
    ...
    #exception clause for Brazil -- ensures heatwave not detected from end of March to November of same year
    exceed_bool[np.isnan(exceed_bool)] = False
    years = np.array([date.fromordinal(int(day)).year for day in t])
    for year in np.unique(years):
        # Find the index of March 31st for the current year
        march_31st = date(year, 3, 31).toordinal()
        if march_31st in t:
            index = np.where(t == march_31st)[0][0]
            exceed_bool = np.insert(exceed_bool, index + 1, False)
            t = np.insert(t, index + 1, march_31st + 1)
